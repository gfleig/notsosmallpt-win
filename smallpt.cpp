...
struct Sphere {
  ...
  double intersect(const Ray &r) const { 
    ...
    double b = op.dot(r.d);         // same as geometric solution found os scratchapixel.com
...
Sphere spheres[] = {
    //     rad, position,                   emission,         color,              material
...
Vec radiance(const Ray &r, int depth, unsigned short *Xi) {
  ...
  Vec n = (x - obj.p).norm();       // center -> intersection vector (pre-normal)
  Vec nl;                           // intersection normal vector

  // inverts n if ray is coming from the inside-out
  if (n.dot(r.d) < 0) {    //Vec nl = n.dot(r.d) < 0 ? n : n * -1;
    nl = n;
  } else {
    nl = n * -1;
  }
  ...
  if (obj.refl == DIFF) {                                                         // Ideal DIFFUSE reflection
    ...
    Vec d = ( u * cos(r1) * r2s + 
              v * sin(r1) * r2s + 
              w * sqrt(1 - r2)      ).norm();    // new random generated direction
    ...
    double nc = 1;          // air refraction
    double nt = 1.5;        // glass refraction
    ...
    Vec tdir = (r.d * nnt - n * ((into ? 1 : -1) * (ddn * nnt + sqrt(cos2t)))).norm();  //couldnt the nl var be used here insteal of n * 1/-1
...
int main(int argc, char *argv[]) {
  ...
  Vec cx = Vec(w * .5135 / h);                //camera "sizes" ?
  Vec cy = (cx % cam.d).norm() * .5135;
  Vec r;                                      //radiance
  Vec *c = new Vec[w * h];                    //image array

  #pragma omp parallel for schedule(dynamic, 1) private(r)    // OpenMP
  for (int y = 0; y < h; y++) {                               // Loop over image rows
    // fprintf(stderr,"\rRendering (%d spp) %5.2f%%",samps*4,100.*y/(h-1)); // *** Commented out for Visual Studio, fprintf is not thread-safe
